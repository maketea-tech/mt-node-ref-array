...
module.exports = function Array(_type, _length) {
  ...
  function ArrayType(data, length) {
    ...
    if (0 === arguments.length) {
      ...
      if (fixedLength > 0) {
        ...
        throw new Error(
          'A "length", "array" or "buffer" must be passed as the first argument'
        );
      }
      ...
      if (data.length < len) {
        throw new Error(
          "array length must be at least " + len + ", got " + data.length
        );
      }
      this.length = len;
      ...
      if (data.length != expectedLength) {
        if (data.length < expectedLength) {
          throw new Error(
            "buffer length must be at least " +
              expectedLength +
              ", got " +
              data.length
          );
  ...
  // save down the "fixedLength" if specified. "mt-node-ref-struct" needs this value
  if (fixedLength > 0) {
  ...
  ArrayType.BYTES_PER_ELEMENT =
    type.indirection == 1 ? type.size : _ref.sizeof.pointer;
 ...
 * Most likely invoked when accessing within a "mt-node-ref-struct" type.
 */
 ...
 * Most likely invoked when setting within a "mt-node-ref-struct" type.
 */
